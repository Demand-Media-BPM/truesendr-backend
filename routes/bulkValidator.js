...
module.exports = function bulkValidatorRouter(deps) {
  ...
  async function buildHistoryForEmail(emailNorm) {
    ...
    // ✅ IMPORTANT: we are only READING training samples here (no writes),
    // so it will NOT increase training data counts.
    ...
    if (stats && stats.sent && stats.sent > 0) {
      ...
      if (domainInvalidRate != null) {
        ...
        // keep same mapping style you already use
    ...
    if (ts) {
      ...
      history.trainingLabel = ts.lastLabel || null; // alias used by merge helper (if any)
  ...
  router.post("/start", maybeBulkAuth, async (req, res) => {
    ...
    const runJob = async ({ streamToRes = false }) => {
      ...
      // ───────────────────────────────────────────────────────────
      // ✅ Yash logic merged into YOUR worker (no deletion of your logic)
      // ───────────────────────────────────────────────────────────
      const worker = async (item) => {
        if (deps.cancelMap?.get(bulkId)) throw new Error("CANCELED");
        ...
        // ✅ Domain flags (same as Yash/single behavior)
        const domain = extractDomain(E);
        ...
        // ✅ If cached is UNKNOWN & domain is bank/healthcare or proofpoint → force live to try SendGrid
        const cachedCatForDecision = cached
          ? String(cached.category || categoryFromStatus(cached.status || "") || "unknown").toLowerCase()
          : null;
        ...
        if (cached && fresh && !forceLiveBecauseCachedUnknown) {
          ...
          // ✅ If Bank/Healthcare OR Proofpoint → try SendGrid FIRST (Yash behavior)
          if (isBankOrHealthcare || isProofpoint) {
            ...
            // (Optional) Domain reputation guard for Bank/Healthcare (Yash behavior)
              ...
              try {
                ...
                if (domainStats && domainStats.sent >= 5) {
                  ...
                  if (bounceRate >= 0.6) {
                    logger(
                      "high_risk_domain",
                      `High bounce domain (${(bounceRate * 100).toFixed(1)}%) → risky`,
                      "warn",
                    );
                    ...
                    await replaceLatest(EmailLog, E, final);
                    await replaceLatest(UserEmailLog, E, final);
                  }
                }
              } catch (e) {
            ...
            // If not already finalized by domain reputation risky, do SendGrid
            if (!final) {
              try {
                const t0 = Date.now();
          ...
          // ✅ If not finalized via SendGrid path, run your existing SMTP flow
          if (!final) {
            const prelimRaw = await validateSMTP(E, {
              logger,
              trainingTag: "bulk",
            ...
            // ✅ SendGrid fallback when SMTP says unknown (Yash behavior)
            if (
              (prelimRaw.category || categoryFromStatus(prelimRaw.status)) === "unknown"
            ) {
              try {
                logger("sendgrid_fallback", "SMTP returned unknown → trying SendGrid fallback", "warn");
                ...
                // continue with SMTP merge below
              }
            }
            ...
            if (!final) {
              ...
              if (catP !== "unknown") {
                ...
                try {
                  ...
                  // ✅ SendGrid fallback for stable unknown (Yash behavior)
                  if (
                    (stableRaw.category || categoryFromStatus(stableRaw.status)) === "unknown"
                  ) {
                    try {
                      logger("sendgrid_stable_fallback", "SMTP stable returned unknown → trying SendGrid fallback", "warn");
                      ...
                      // continue with SMTP stable merge below
                    }
                  }
