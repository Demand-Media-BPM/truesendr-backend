...
//   outlook: {
//     label: "Outlook",
//     emailEnv: "DELIV_OUTLOOK_EMAIL",
//     passEnv: null,
//     imap: { host: "outlook.office365.com", port: 993, secure: true },
//     smtp: { host: "smtp-mail.outlook.com", port: 587, secure: false },
//     inboxFolder: "INBOX",
//     spamFolder: "Junk",
//     spamFolderAlternates: ["Junk Email"],
//   },
// };
...
//     .trim()
//     .toLowerCase();
//   return k === "microsoft_business" || k === "hotmail" || k === "outlook";
// }
...
//       : providerKey === "outlook"
//         ? process.env.DELIV_OUTLOOK_REFRESH_TOKEN
...
//         : providerKey === "outlook"
//           ? "Microsoft OAuth env missing for Outlook. Need DELIV_MS_CLIENT_ID, DELIV_MS_CLIENT_SECRET, DELIV_MS_REDIRECT_URI, DELIV_OUTLOOK_REFRESH_TOKEN"
...
//         // ✅ Use IMAP special mailbox name INBOX and EXAMINE (read-only)
//         const targetInbox =
//           String(inboxFolder || "").toUpperCase() === "INBOX"
//             ? "INBOX"
//             : inboxFolder;

//         sendLine(`${examineTag} EXAMINE ${targetInbox}`);
//         setWaiter(
//           examineTag,
//           () => {
...
//             );
//           },
//           (line) => finishErr(new Error(`EXAMINE failed before LIST: ${line}`)),
//         );
//       } catch (e) {
//         finishErr(e);
//       }
...
//       // Microsoft providers: use ImapFlow for folder listing
//       if (isMs) {
//         const client = new ImapFlow({
//           host: cfg.imap.host,
//           port: cfg.imap.port,
  ...
  return await new Promise((resolve, reject) => {
    ...
    async function startAuthAndList() {
      try {
        ...
        const noopTag = "n" + tagN++;
        const listTag1 = "l" + tagN++;
        const listTag2 = "m" + tagN++;

        // Step 1: stabilize authenticated state
        sendLine(`${noopTag} NOOP`);
        setWaiter(
          noopTag,
          () => {
            // Step 2: try safer one-level listing first
            sendLine(`${listTag1} LIST "" "%"`);
            setWaiter(
              listTag1,
              () => {
                try {
              ...
              () => {
                // Step 3: fallback to broader pattern
                sendLine(`${listTag2} LIST "" "*"`);
                setWaiter(
                  listTag2,
                  () => {
                    try {
          ...
          (line) => finishErr(new Error(`NOOP failed before LIST: ${line}`)),
        );
      } catch (e) {
        finishErr(e);
      }
...
module.exports = function deliverabilityRouter(deps = {}) {
  ...
  router.post("/list-folders", async (req, res) => {
    ...
    try {
      ...
      // Microsoft providers: use custom XOAUTH2 IMAP path (same family as /test-connection)
      if (isMs) {
        try {
          const accessToken = auth?.accessToken;
          if (!accessToken) {
          ...
          const folders = await msImapListFolders({
            ...
            accessToken,
            timeoutMs: 60_000,
            inboxFolder: cfg.inboxFolder || "INBOX",
          });
