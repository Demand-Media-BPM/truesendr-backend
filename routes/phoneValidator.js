...
// function applyMissingCarrierInvalidRule(payload, inputCountryCode) {
//   const cc = String(inputCountryCode || payload.country || "")
//     .trim()
//     .toUpperCase();

//   const isCA = cc === "CA";

//   // "provider" in your wording = carrier (network/provider)
//   const carrier = (payload.carrier || "").trim();

//   // ✅ Except Canada: if carrier missing => force invalid
//   if (!isCA && !carrier) {
...
//       // ✅ Apply new trick: Except CA, if carrier missing => force invalid
//       let effective = {
//         valid: Boolean(globalHit.valid),
//         country: globalHit.country || null,
//         e164: globalHit.e164 || phoneSanitized,
//         carrier: globalHit.carrier || null,
//         lineType: globalHit.lineType || null,
//         callerName: globalHit.callerName || null,
//         ownerType: globalHit.ownerType || null,
//       };

//       effective = applyMissingCarrierInvalidRule(effective, cc);

//       // Recompute lead quality based on effective validity
//       const {
//         score: leadQualityScore,
//         percentage: leadQualityPercentage,
//         band: leadQualityBand,
//       } = computeLeadQuality(effective, cc);

//       // Touch global timestamp + keep cache consistent with new rule
...
//             // ✅ backfill corrected validity + scoring
...
//             lineType: effective.lineType,
//             callerName: effective.callerName,
...
//       return res.json({
//         ok: true,
//         phone: effective.e164 || phoneSanitized,
//         valid: Boolean(effective.valid),
//         carrier: effective.carrier,
//         lineType: effective.lineType,
//         country: effective.country,
//         callerName: effective.callerName,
...
//       // ✅ Apply new trick: Except CA, if carrier missing => force invalid
//       let effective = {
//         valid: Boolean(userHit.valid),
//         country: userHit.country || null,
//         e164: userHit.e164 || phoneSanitized,
//         carrier: userHit.carrier || null,
//         lineType: userHit.lineType || null,
//         callerName: userHit.callerName || null,
//         ownerType: userHit.ownerType || null,
//       };
...
//       // ✅ Touch tenant timestamps + persist corrected validity/scoring (NO new record)
...
//     // ✅ Apply new trick: Except CA, if carrier missing => force invalid
//     const effective = applyMissingCarrierInvalidRule(normalized, cc);
...
router.post("/validate", requireAuth, async (req, res) => {
  try {
    ...
    const rawInputNumber = String(phone).trim(); // local number user typed

    let localDigits = rawInputNumber.replace(/[^\d]/g, "");
    const dialDigits = String(dialCode).trim().replace(/[^\d]/g, "");

    // ✅ If user pasted number starting with country code, strip it once
    ...
    // final E.164 for Twilio
    const e164Input = `+${dialDigits}${localDigits}`;

    // keep old variable names so existing logic doesn't break
    const phoneSanitized = e164Input;

    // you don't have ISO country anymore from UI, so use dial as "country code identity"
    // (or keep null if you prefer)
    let cc = null;

    // stable cache key based on dial + local (digits only)
    const lookupKey = `${dialDigits}:${localDigits}`;

    // store dial on rows using existing field (minimal change)
    const inputDial = `+${dialDigits}`;
    ...
    if (globalHit) {
      ...
      effective = applyMissingCarrierInvalidRule(effective, effective.country);
      ...
      const {
        ...
        percentage: leadQualityPercentage,
        band: leadQualityBand,
      } = computeLeadQuality(effective, effective.country);
      ...
      return res.json({
        ok: true,
        phone: effective.e164 || e164Input,
        valid: Boolean(effective.valid),
        carrier: effective.carrier,
        lineType: effective.lineType,
        country: effective.country,
    ...
    if (DEV_PHONE_LOG) {
      console.log("───────────────────────────────");
      console.log("[PhoneValidator] Twilio request:");
      console.log("  phone        :", e164Input);
      console.log("  fields       :", fetchOptions.fields || "(none)");
      console.log("  dialCode     :", inputDial);
      console.log("  e164         :", e164Input);
      console.log("  lookupKey    :", lookupKey);
    }
    ...
    const normalized = {
      valid: twilioData.valid === undefined ? true : Boolean(twilioData.valid),
      country: twilioData.countryCode || null, // ISO like "US", "CA"
      e164: twilioData.phoneNumber || e164Input,
      carrier: lti.carrier_name || twilioData.carrier?.name || null,
      lineType: lti.type || twilioData.carrier?.type || null,
      callerName: caller.caller_name || null,
      ownerType: caller.caller_type || null,
    ...
    // ✅ IMPORTANT: set cc from Twilio ISO country
    cc = normalized.country;

    // ✅ Apply rule using ISO country (Canada exception works correctly)
    const effective = applyMissingCarrierInvalidRule(normalized, cc);

    // ✅ Compute lead quality using correct country
    const {
      score: leadQualityScore,
      percentage: leadQualityPercentage,
      band: leadQualityBand,
